...
export function TerritoryBanner({ scope = "user" }: { scope?: "user" | "club" }) {
  ...
  return (
    <section className="relative rounded-3xl overflow-hidden ring-1 ring-overlay/10">
      ...
      <div className="relative p-6 sm:p-8">
        ...
        <p className="mt-3 text-sm sm:text-base text-foreground/70 max-w-2xl">
          {isLeader
            ? <>On porte le {t.zone} en tête. {t.rivals[1]?.clubName} suit à <strong className="text-foreground">{compact(me.weekPoints - t.rivals[1].weekPoints)} pts</strong>. Continuons ensemble.</>
            : <>#{t.myClubRank} sur {t.totalClubsInZone} clubs du {t.zone}. <strong className="text-foreground">{leader.clubName}</strong> est devant de <strong className="flame-text">{compact(gap)} pts</strong>. {scope === "user" ? "Chaque minute compte." : "L'équipe est mobilisable."}</>
          }
        </p>
              ...
              <div
                ...
                className={`rounded-2xl p-3 ring-1 transition ${
                  isMine
                    ...
                    : i === 0
                    ? "bg-overlay/10 ring-overlay/15"
                    : "bg-overlay/5 ring-overlay/10"
                }`}
              >
                <div className="flex items-center gap-2">
                  <div className={`w-9 h-9 rounded-xl grid place-items-center font-black text-xs ${isMine ? "flame-gradient text-black" : "bg-overlay/10 text-foreground"}`}>
                    {r.logo}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-foreground/50 truncate">{r.members.toLocaleString("fr-FR")} membres</p>
                  </div>
                </div>
                <div className="mt-2 flex items-center justify-between text-[11px]">
                  ...
                  {r.trend === 0 && <span className="text-foreground/40 inline-flex items-center"><Minus size={10} /></span>}
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-5 flex items-center gap-2 text-[12px] text-foreground/60">
